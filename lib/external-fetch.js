 ...
 */
exports.movieList = function(list_slug, cb){
  async.waterfall([
 ...
 * and adds or updates them the Database.
 */
exports.movie = function(imdb_id, cb){
...
var addEachMovietoDB = function(movies, cb) {
  ...
  async.each(movies, function(movie, next_movie){
    ...
    Model.Movie.findOne({'ids.imdb': movie.ids.imdb}, function(err, db_movie){
      if (err) return next_movie(new VError(err, 'Could not fetch ' + movie.title + ' from DB. (imdb_id: ' + movie.ids.imdb + ')'));
      ...
      // movie doesn't already exists
        ...
        movie.save(function (err, product) {
          ...
          debug('saved ' + movie.title + ' in database');
          next_movie();
        });
      }
    });
