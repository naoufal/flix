...
var redisSet = require('./redis-functions').set;
...
var addEachMovietoDB = function(movies, cb) {
  async.mapSeries(movies, function(movie, next_movie){
    ...
    Model.Movie.findOne({'ids.imdb': movie.ids.imdb}, function(err, db_movie){
      ...
      if (db_movie && db_movie.ids.tmdb) {
        ...
        db_movie.update({
          ...
          debug('updated ' + movie.title + ' in database');
          next_movie(null, movie.ids.imdb);
        });
      // movie doesn't already exists
      } else {
        // add it to db
        ...
        movie.save(function (err, product) {
          ...
          setTimeout(function(){
            movieFetch(movie.ids.imdb, function(err, movie){
              if (err) return next_movie(null, null);
              next_movie(null, movie.ids.imdb);
            });
          }, 1000);
          // next_movie();
  ...
  }, function afterEach(err, movie_imdb_ids){
    if (err) return cb(err);
    var movie_list = _.without(movie_imdb_ids, null);
